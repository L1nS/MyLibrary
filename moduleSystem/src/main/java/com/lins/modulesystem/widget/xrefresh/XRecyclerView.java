...
public class XRecyclerView extends RecyclerView {
    ...
    private XRefreshHeader mRefreshHeader;
    ...
    public void addHeaderView(View view) {
        if (pullRefreshEnabled && !(mHeaderViews.get(0) instanceof XRefreshHeader)) {
            XRefreshHeader refreshHeader = new XRefreshHeader(getContext());
            mHeaderViews.put(0, refreshHeader);
            mRefreshHeader = refreshHeader;
        }
    ...
    public void noMoreLoadingWithoutFoot() {
        isLoadingData = false;
        final View footView = mFootViews.get(0);
        isnomore = true;
        if (footView instanceof LoadingMoreFooter) {
    ...
    public void onScrollStateChanged(int state) {
        ...
        if (state == RecyclerView.SCROLL_STATE_IDLE && mLoadingListener != null && !isLoadingData && loadingMoreEnabled) {
            ...
            if (layoutManager.getChildCount() > 0
                    && lastVisibleItemPosition >= layoutManager.getItemCount() - 1 && layoutManager.getItemCount() > layoutManager.getChildCount() && !isnomore && mRefreshHeader.getState() < XRefreshHeader.STATE_REFRESHING) {
        ...
        switch (ev.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (isOnTop() && pullRefreshEnabled) {
                    ...
                    if (mRefreshHeader.getVisiableHeight() > 0 && mRefreshHeader.getState() < XRefreshHeader.STATE_REFRESHING) {
                        return false;
                    }
                }
                break;
